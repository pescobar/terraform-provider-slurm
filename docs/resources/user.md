...
| `association.allowed_qos` | **No** — intentionally skipped (see below) |
| All other limit fields (`max_jobs_accrue`, TRES limits, etc.) | **No** — null until reconcile |

### Why `allowed_qos` is not read during import

The `allowed_qos` field in an association block is intentionally never
populated during `import`. Slurm enforces a constraint that a user's
`default_qos` must appear in their `allowed_qos` list. If the provider read
the actual QOS list from Slurm into state but your config does not declare an
`allowed_qos` list, the reconcile apply would attempt to clear it. Slurm
rejects that request with:
...
Leaving `allowed_qos` null after import avoids this: the reconcile only
changes what config explicitly declares, and does not touch the existing QOS
list for associations where `allowed_qos` is absent from config.
